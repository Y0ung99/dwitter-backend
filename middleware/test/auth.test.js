...
describe('Auth Middleware', () => {
    ...
    it('returns 401 for the request with Authorization header but doesnt start with Bearer', async () => {
        const request = httpMocks.createRequest({
            method: 'GET',
            url: '/tweets',
            headers: {Authorization: 'Baarer 4123'},
        });
        const response = httpMocks.createResponse();
        const next = jest.fn();
        ...
        expect(request.get('Authorization')).toBe('Baarer 4123')
        expect(response.statusCode).toBe(401);
        expect(response._getJSONData().message).toBe('Authentication Error');
        expect(next).not.toBeCalled();
    });

    it('returns 401 for the request when verify incorrect jwt', async () => {
    ...
    it('returns 401 for the request when verify jwt and then find incorrect user', async () => {
        const token = faker.random.alphaNumeric(128);
    ...
    it('success authorization, correct jwt, correct user', async () => {
        const token = faker.random.alphaNumeric(128);
        const userId = faker.random.alphaNumeric(32);
        const request = httpMocks.createRequest({
            method: 'GET',
